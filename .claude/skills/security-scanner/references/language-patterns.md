...
Load the sections that match the stack you found in Step 1. Each section lists the entry points where
untrusted data comes in, the sinks where it becomes dangerous, and common traps for each framework.
...
**React** — `dangerouslySetInnerHTML`; `href`/`src` set from user input, which allows `javascript:`;
secrets in `REACT_APP_*`/`NEXT_PUBLIC_*` (sent to the browser); `localStorage` holding
tokens (XSS can read them — use `HttpOnly` cookies instead).

**Next.js** — `getServerSideProps` leaking server-only data into props; API routes without
auth checks; middleware matchers that miss protected paths; server actions that trust
...
**ORM raw-query helpers** — `sequelize.literal`, `knex.raw`, TypeORM
...
**FastAPI** — auth dependency defined but not used on every route; response
...
`@CrossOrigin("*")` with credentials; `permitAll()` patterns that allow more than planned;
`@PreAuthorize` missing on service methods that other paths can reach; SpEL evaluation of user
...
production; policies defined but never called (`authorize()` missing).
...
changing shared state without a mutex (race in auth/balance logic).
...
in release mode (use `checked_add`/`saturating_*` for money or index math);
...
controller but `[AllowAnonymous]` left on an action; over-posting into EF entities (bind to a
separate DTO); `ServerCertificateValidationCallback` returning `true`; secrets in
